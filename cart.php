...
if(isset($_POST['form1'])) {
    ...
    for($i = 1; $i <= count($arr1); $i++) {
        // Thực hiện truy vấn để lấy quantity từ tbl_product_quantity
        $statement = $pdo->prepare("SELECT pq.quantity, s.size_name, c.color_name 
                                     FROM tbl_product_quantity pq
                                     INNER JOIN tbl_size s ON pq.size_id = s.size_id
                                     INNER JOIN tbl_color c ON pq.color_id = c.color_id
                                     WHERE pq.product_id = ? AND pq.size_id = ? AND pq.color_id = ?");
        $statement->execute(array($arr1[$i], $_SESSION['cart_size_id'][$i], $_SESSION['cart_color_id'][$i]));
        ...
        if($product_quantity < $arr2[$i]) {
            $allow_update = 0;
            $message .= '"' . $arr3[$i] . '" (Kích thước: ' . $size_name . ', Màu sắc: ' . $color_name . ') không có đủ "' . $arr2[$i] . '" sản phẩm trong kho hàng\n';
        } else {
				...
				<div class="cart">
                    <table class="table table-responsive table-hover table-bordered">
                        <tr>
                            ...
                            <th class="text-right"><?php echo 'Tổng giá sản phẩm' ?></th>
                        ...
                        <tr>
                            ...
                            <td><?php echo number_format($arr_cart_p_current_price[$i], 0, '.', ',') ?><?php echo LANG_VALUE_1; ?></td>
                            <td>
                            ...
                            <td class="text-right">
                                ...
                                ?>
                                <?php echo number_format($row_total_price, 0, '.', ',') ?><?php echo LANG_VALUE_1; ?>
                            </td>
                        ...
                        <?php endfor; ?>
                        <tr>
                            <th colspan="7" class="total-text">Tổng giá</th>
                            <th class="total-amount"><?php echo number_format($table_total_price, 0, '.', ',') ?><?php echo LANG_VALUE_1; ?></th>
                            <th></th>
                        </tr>
                    </table> 
                </div>
