...
if(isset($_POST['form_add_to_cart'])) {
	...
	$current_p_qty = 0; // Đặt giá trị mặc định cho $current_p_qty

    $statement = $pdo->prepare("SELECT quantity FROM tbl_product_quantity WHERE product_id = ? AND size_id = ? AND color_id = ?");
    $statement->execute(array($_REQUEST['id'], $_POST['size_id'], $_POST['color_id']));
    $result = $statement->fetchAll(PDO::FETCH_ASSOC);							
    foreach ($result as $row) {
        $current_p_qty = $row['quantity'];
    }

    $p_qty = isset($_POST['p_qty']) ? $_POST['p_qty'] : 0; // Lấy giá trị từ $_POST hoặc đặt mặc định là 0 nếu không có giá trị

	if($_POST['p_qty'] > $current_p_qty):
		$temp_msg = 'Số lượng bạn đặt lớn hơn số lượng trong kho của chúng tôi';
		?>
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (isset($_POST['size_id']) && isset($_POST['color_id']) && isset($_POST['product_id'])) {
        // Lấy dữ liệu từ form
        ...
        // Thực hiện truy vấn SQL, ví dụ:
        $statement = $pdo->prepare("SELECT quantity FROM tbl_prodcut_quantity WHERE product_id = ? AND size_id = ? AND color_id = ?");
        $statement->execute([$product_id, $size_id, $color_id]);
        $result = $statement->fetch(PDO::FETCH_ASSOC);

        if ($result) {
            // Hiển thị số lượng sản phẩm
            echo "Số lượng sản phẩm: " . $result['quantity'];
        } else {
            echo "Không có thông tin số lượng sản phẩm.";
        }
		...
		<div class="row">
			<div class="col-md-12">
                <div class="breadcrumb mb_30">
                    <ul>
                        <li><a href="<?php echo 'index.php' ?>">Trang chủ</a></li>
                        <li>></li>
					...
					<div class="row">
						...
						<div class="col-md-7">
                            ...
                            <div style="margin-bottom: 10px">
                                <h4 style=" display: contents; font-size: 16px">Lượt xem: </h4> <span style="font-weight: 700; font-size: 16px" ><?php echo $p_total_view?></span>
                            </div>

                            <div style="margin-bottom: 10px">
                                <h4 style="display: contents; font-size: 16px">Tồn kho: </h4>
                                <span id="available_quantity" style="font-weight: 700; font-size: 16px"></span>
                            </div>
                            

                            <form action="" method="post" onsubmit="return validateBeforeAddToCart()">
                            <div id="id_qty_do" class="p-quantity">
                                <div class="row">
                                    <?php if(isset($size)): ?>
                                    <div class="col-md-12 mb_20">
                                        <?php echo LANG_VALUE_52; ?> <br>
                                        <select name="size_id" class="form-control select2" style="width:auto;">
                                        <option value="placeholder" selected disabled hidden>Chọn kích thước</option>
                                            <?php
                                    ...
                                    <div class="col-md-12">
                                        ...
                                        <select name="color_id" class="form-control select2" style="width:auto;">
                                            <option value="" disabled selected hidden>Chọn màu sắc</option> <!-- Dòng mặc định -->
                                            <?php
                                ...
                                <button type="button" onclick="getQuantity()" style="background-color: #4CAF50; color: white; padding: 5px 10px; border: none; border-radius: 4px; cursor: pointer; transition: background-color 0.3s; margin-top:20px" onmouseover="this.style.backgroundColor='#45a049'" onmouseout="this.style.backgroundColor='#4CAF50'">Xem tồn kho</button>
                            </div>


							<div class="p-price">
                                <span style="font-size:14px;"><?php echo LANG_VALUE_54; ?></span><br>
                                <?php if ($p_current_price < 0): ?>
                                <span>
                                    <?php echo number_format($p_current_price, 0, '.', ',')?>₫
                                </span>
                                <?php else: ?>
                                <span>
                                    <?php if($p_current_price != 0): ?>
                                        ...
                                        <?php if($p_old_price != ''): ?>
                                            <del>
                                                <?php echo number_format($p_old_price, 0, '.', ',')?>₫
                                            </del>
                                        <?php endif; ?>
                                    <?php else: ?>
                                        <?php echo number_format($p_old_price, 0, '.', ',')?>₫
                            ...
                            <script>
                            let viewedInventory = false;

                                function getQuantity() {
                                    // Lấy giá trị size_id, color_id và product_id từ form
                                    const sizeId = document.querySelector("select[name='size_id']").value;
                                    const colorId = document.querySelector("select[name='color_id']").value;
                                    ...
                                    xhr.onreadystatechange = function() {
                                        if (xhr.readyState === XMLHttpRequest.DONE) {
                                            if (xhr.status === 200) {
                                                // Nhận dữ liệu từ server và hiển thị số lượng sản phẩm
                                                document.getElementById("available_quantity").textContent = xhr.responseText;
                                            } else {
                                                // Xử lý lỗi
                                                document.getElementById("available_quantity").textContent = "Không thể lấy thông tin số lượng sản phẩm.";
                                            }
                                        }
                                    };

                                    // Gửi yêu cầu POST để lấy số lượng sản phẩm
                                    xhr.open("POST", "get_quantity.php", true);
                                    xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
                                    xhr.send("product_id=" + productId + "&size_id=" + sizeId + "&color_id=" + colorId);
                                ...
                                function validateBeforeAddToCart() {
                                    ...
                                    const buttonShow_inventory = document.querySelector("select[name='form_add_to_cart']");

                                    if (!viewedInventory) {
                                        alert('Vui lòng xem tồn kho trước khi thêm vào giỏ hàng.');
                                        return false;
                                    }
                                    
                                    if (sizeSelect.value === '' || colorSelect.value === '' ) {
                                        alert('Vui lòng chọn kích thước và màu sắc trước khi thêm vào giỏ hàng.');
                                        return false; // Ngăn không cho submit nếu chưa chọn đủ
                                    }
                                    return true; // Cho phép thực hiện submit nếu đã chọn đủ
                                }
                            </script>
						</div>
					</div>
					...
					<div class="row">
						<div class="col-md-12">
							...
							<div class="tab-content">
								...
								<div role="tabpanel" class="tab-pane" id="review" style="margin-top: -30px;"></div>
